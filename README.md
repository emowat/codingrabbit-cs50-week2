...
If you don't already have a GitHub account, you must create one to use GitHub Codespaces:
...
## Step 3: Start Your Problem Sets
Inside this repository, you will find several problem set folders for Week 2:
* `scrabble/` - Calculate point scores for words in a Scrabble-like game.
* `readability/` - Compute the reading level of a text using the Coleman-Liau index.
* `caesar/` - Encrypt messages using a Caesar cipher. (Easier version)
* `substitution/` - Encrypt messages using a Substitution cipher. (More comfortable version)

**Note:** For the encryption assignment, you only need to submit *one* of them. Choose either `caesar` (if you are feeling less comfortable) or `substitution` (if you are feeling more comfortable).

Each folder contains a `README.md` with specific instructions and the exact terminal commands you need to get started.
...
2. Follow the instructions to create your `.c` files and begin working!
...
1. `git remote add upstream https://github.com/emowat/codingrabbit-cs50-week2.git` *(one-time setup — links you to the original course repository; skip this if you've already done it once)*
